...
void Player::SetCurrentField(Field* curField, Field* targetField)//dodac przesuwanie w lewo jesli pole jest juz zajete - po to jest curField
{
	...
	xPos = targetField->getxPos();
	yPos = targetField->getyPos();
}

void Player::makeMove(Field** field)
{
	if (daysJailed > 0) { daysJailed--; std::cout << "czekasz jeszcze " << daysJailed << " dni\n"; return; }
	...
	if (dice1 == dice2 && dicesMatched == true) {
		std::cout << "DUBLET = wiezienie\n" << currentField << std::endl; daysJailed = 2; dicesMatched = false;
		SetCurrentField(field[currentField], field[10]); return;
	}
	int fieldMod = currentField + total;
	if (fieldMod >= 40) { fieldMod %= 40; money += 400; }
	SetCurrentField(field[currentField], field[fieldMod]);

	if (fieldMod == 30) { std::cout << "IDZIESZ DO WIEZIENIA!\n"; SetCurrentField(field[currentField], field[10]); daysJailed = 2; }
}
