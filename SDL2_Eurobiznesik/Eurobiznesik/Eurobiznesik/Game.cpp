...
void Game::init(const char* title, int xPos, int yPos, int width, int height, bool fullscreen)//przygotowuje srodowisko, tworzy okno, tworzy renderer
{
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0)
	{
		...
		std::cout << "Initial load - dziala\n";
		window = SDL_CreateWindow(title, xPos, yPos, width, height, flag);
		if (window)
			std::cout << "Okno - dziala\n";
		renderer = SDL_CreateRenderer(window, -1, 0);
		if (renderer)
		{
			SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
			std::cout << "Renderer - dziala\n";
		}
		isRunning = true;
	}
	else
	...
	{
		playerList[i] = new Player(playerTextures[i], spawnX, spawnY, w, h,1);
		spawnX -= width/64;
	}
	map = new Map(w, h);
...
void Game::handleEvents()
{
	...
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_MOUSEBUTTONDOWN:
			std::cout << "ROZMIAR WEKTORA(ILOSC POSIADLOSCI KUPIONYCH) = " << playerList[0]->getFields().size() << std::endl;
			break;
