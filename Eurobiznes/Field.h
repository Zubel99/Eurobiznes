...
private:
	...
	bool buyingLocked;
	bool rebuying;
	int playersOccupying;
	std::vector<Player*> playersOnField;//temp
...
public:
	...
	void addPlayersOnField(Player* newPlayer) { playersOnField.push_back(newPlayer); }
	void subPlayersOnField(Player* newPlayer);
	void fixPlayerPosition();


	void addPlayersOccupying(int amount) { playersOccupying += amount; }
	int getPlayersOccupying() { return playersOccupying; }
